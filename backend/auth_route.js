...
auth_router.post("/register", async (req, res) => {
  try {
    ...
    // Postgres: use $1, $2 and result.rows
    const { rows: existing } = await database.query(
      "SELECT * FROM users_login WHERE email = $1 OR phonenumber = $2 LIMIT 1",
      [email, phonenumber]
    );
    ...
    await database.query(
      "INSERT INTO users_login (name, password, email, phonenumber, gender) VALUES ($1, $2, $3, $4, $5)",
...
auth_router.post("/signin", async (req, res) => {
  try {
    ...
    const { rows } = await database.query(
      "SELECT * FROM users_login WHERE email = $1 LIMIT 1",
      [email]
    );
...
auth_router.post("/change_password", async (req, res) => {
  try {
    ...
    if (!token || !secret_key) {
      return res
        .status(401)
        .json({ message: "Token or secret key not available" });
    }
    ...
    await database.query(
      "UPDATE users_login SET password = $1 WHERE id = $2",
      [hashedPassword, verify.id]
    );
    ...
    res
      .status(401)
      .json({ message: "password change failed", error: error });
  }
});
...
auth_router.post("/delete-account", async (req, res) => {
  ...
  try {
    ...
    await database.query("DELETE FROM users_login WHERE id = $1", [userid]);
